"""
train_coral_focal_efficientnetb4.py
====================================
EfficientNet-B4 CORAL experiment using the same APTOS 2019 + EyePACS supplement
data and focal-weighted ordinal loss (CORALModule_v5) as v7.  Backbone replaced
from DenseNet121 to EfficientNet-B4 (1792-d features); everything else kept
identical.
"""
...
# -- MODEL ---------------------------------------------------------------------
class EfficientNetB4withCORAL(nn.Module):
    def __init__(self, num_classes=5, gamma=2.0, pretrained=True, lambda_ord=0.1):
        ...
        self._gradient   = None
        # Hook on last conv block (features[-1]) for Grad-CAM
...
def freeze_early_layers(model):
    for i in range(4):  # features[0..3], equiv. to denseblock1+2
...
# -- CONFIG --------------------------------------------------------------------
CONFIG = {
    'model_name':           'efficientnetb4_coral',
    'num_classes':          5,
    'input_size':           224,
    'batch_size':           16,
    'num_epochs':           30,
    ...
    'data_dir':             './data/raw',
    'output_dir':           os.path.join('results', 'exp_efficientnetb4'),
...
def main():
    print("=" * 80)
    print("TRAINING efficientnetb4: EfficientNet-B4 + CORAL...")
    print("=" * 80)
    ...
    report_full = (
        "Classification Report -- exp_efficientnetb4\n"
        "=" * 60 + "\n"
    ...
    summary = {
        'model':             'EfficientNet-B4 + independent CORAL projections',
        'experiment':        'exp_efficientnetb4',
        'changes_vs_v7':     [
            'Backbone: DenseNet121 -> EfficientNet-B4 (1792-d features)',
            'Frozen: features[0:4] of 9 blocks (equiv. to denseblock1+2)',
        ],
