...
class PreprocessedDRDataset(Dataset):
    """
    Loads preprocessed images from cache and applies augmentations.
    
    preprocess_v3 augmentations already include:
      - Normalize(ImageNet mean/std)
      - ToTensorV2()
    So __getitem__ returns tensors directly (channels-first, normalized, in [0, 1]).
    """
    ...
    def __getitem__(self, idx):
        ...
        # Apply augmentation (includes Normalize + ToTensorV2)
        if self.augmentation:
            output = self.augmentation(image=image)
            image = output['image']
            # output['image'] is already a torch.Tensor (channels-first, normalized)
        else:
            # Fallback: manual conversion if no augmentation
...
def main():
    ...
    train_dataset = PreprocessedDRDataset(
        CONFIG['cache_dir'], train_df_balanced, augmentation=get_train_augmentation_strong()
    )
