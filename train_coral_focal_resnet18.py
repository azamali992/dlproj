"""
train_coral_focal_resnet18.py
=============================
ResNet18 CORAL experiment using the same APTOS 2019 + EyePACS supplement data
and focal-weighted ordinal loss (CORALModule_v5) as v7.  Backbone replaced from
DenseNet121 to ResNet18 (512-d features); everything else kept identical.
"""
...
# -- MODEL ---------------------------------------------------------------------
class ResNet18withCORAL(nn.Module):
    def __init__(self, num_classes=5, gamma=2.0, pretrained=True, lambda_ord=0.1):
        ...
        self._gradient   = None
        # Hook on last BasicBlock of layer4 for Grad-CAM
...
CONFIG = {
    ...
    'data_dir':             './data/raw',
    'output_dir':           os.path.join('results', 'exp_resnet18'),
...
def main():
    print("=" * 80)
    print("TRAINING resnet18: ResNet18 + CORAL (same data/loss as v7)")
    print("=" * 80)
    ...
    report_full = (
        "Classification Report -- exp_resnet18\n"
        "=" * 60 + "\n"
    ...
    summary = {
        'model':             'ResNet18 + independent CORAL projections',
        'experiment':        'exp_resnet18',
        'changes_vs_v7':     [
            'Backbone: DenseNet121 -> ResNet18 (512-d features)',
