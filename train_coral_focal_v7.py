"""
train_coral_focal_v7.py
=======================
Identical to exp_v6, with one addition:

  50 images per DR grade from the EyePACS dataset are merged into the
  training split BEFORE oversampling.  The EyePACS images live in
  data/eyepacs_supplement/ and are preprocessed with the same hybrid
  pipeline used in v6.  No original APTOS file is modified.

Run `python prepare_eyepacs_supplement.py` once before this script.

All model/loss/training decisions identical to exp_v6:
- DenseNet121 + CORALModule_v5 (4 independent projections)
- Alpha computed on ORIGINAL imbalanced APTOS labels (same as v6)
...
# -- CONFIG --------------------------------------------------------------------
CONFIG = {
    'model_name':           'densenet121_coral_v7',
    'num_classes':          5,
    'input_size':           224,
    'batch_size':           16,
    'num_epochs':           30,
...
def build_dataframes():
    """
    Returns (aptos_train_df, aptos_val_df, combined_train_df) where
    ...
    """
    # APTOS split (same seed / ratio as v6 for a fair comparison)
    ...
    # EyePACS supplement (all 250 images go into training only, never validation)
...
def main():
    print("=" * 80)
    print("TRAINING v7: v6 + EyePACS supplement (50 images/class)")
    print("=" * 80)
    ...
    # Pass only the columns create_balanced_train_dataframe needs
    # (id_code, diagnosis) -- it ignores extra columns, so we pass full df
    train_df_balanced = create_balanced_train_dataframe(
        train_df=combined_train_df,
        strategy='oversample',
        target_per_class=CONFIG['target_per_class'],
    )
    # Re-attach img_dir after oversampling (oversample duplicates rows, preserving all columns)
    ...
    freeze_early_layers(model, blocks=('denseblock1', 'denseblock2'))

    # Alpha on ORIGINAL imbalanced APTOS labels (same as v6 -- not inflated by supplement)
    ...
    summary = {
        'model':          'DenseNet121 + independent CORAL projections (v7)',
        'experiment':     'exp_v7',
        'changes_vs_v6':  [
            f'EyePACS supplement: {len(supp_df)} images ({len(supp_df)//5}/class)',
            'Supplement preprocessed with same hybrid pipeline as v6',
            'Alpha still computed on APTOS-only labels for consistency',
            'Validation is APTOS-only -- directly comparable to v6',
        ],
