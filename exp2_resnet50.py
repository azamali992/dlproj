"""
EXPERIMENT 2: ResNet50 + Cross-Entropy (Baseline)
===================================================
Change from Exp1: ResNet18 → ResNet50 (deeper, 25M vs 11M params)
Everything else SAME as baseline.

Purpose: Does a deeper model help?
"""
...
class Config:
    ...
    IMAGE_SIZE = 224
    BATCH_SIZE = 8  # Smaller batch for ResNet50 (more params, 4GB GPU)
    NUM_CLASSES = 5
    NUM_EPOCHS = 10
    LEARNING_RATE = 1e-4
    VAL_SPLIT = 0.2
    DEVICE = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    RESULTS_DIR = 'results/exp2_resnet50'
    EXP_NAME = 'Exp2: ResNet50 + CE'
...
print(f"Model: ResNet50 ({sum(p.numel() for p in model.parameters()):,} params)")
