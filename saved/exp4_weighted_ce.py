"""
EXPERIMENT 4: ResNet18 + Weighted Cross-Entropy
=================================================
Change from Exp1: CE → Weighted CE (inverse frequency class weights)
Everything else SAME as baseline.

Purpose: Does simply weighting the loss by class frequency help minority classes?

Why: Grade 0 has 1805 images, Grade 3 has only 193.
     Without weights, model ignores rare classes.
     Weighted CE gives rare classes higher loss → model pays more attention.
"""
...
class Config:
    ...
    RESULTS_DIR = 'results/exp4_weighted_ce'
    EXP_NAME = 'Exp4: ResNet18 + Weighted CE'
...
class_weights = class_weights / class_weights.sum() * config.NUM_CLASSES  # Normalize so mean=1
...
criterion = nn.CrossEntropyLoss(weight=class_weights)  # ← WEIGHTED!
...
with open(os.path.join(config.RESULTS_DIR, 'config.txt'), 'w') as f:
    ...
    f.write(f"Preprocessing: Resize+Normalize\nAugmentation: None\nImage Size: {config.IMAGE_SIZE}\n")
