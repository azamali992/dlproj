"""
train_coral_focal_v8.py
=======================
APTOS 2019 + EyePACS supplement, denseblock1-only freeze.

Differences from exp_v7:
  - v7 froze denseblock1 AND denseblock2.
  - v8 freezes denseblock1 ONLY (same as v5/v6), letting denseblock2 train.
    This gives the model more capacity to adapt to the expanded dataset.

Everything else is identical to v7:
  DenseNet121 + CORALModule_v5, grade-aware augmentation, OneCycleLR,
  grad clip 1.0, TTA, top-3 ensemble, Grad-CAM analysis.

================================================================================
TEAMMATE SETUP — read this before running
================================================================================

This experiment uses images from both the APTOS 2019 dataset (already in the
repo) and a supplement drawn from EyePACS (NOT in the repo — too large).
Follow these steps once before running the training script.
...
--------------------------------
  The project root has a .env file with:
    KAGGLE_API_TOKEN=KGAT_...

  Run this once to write your access token to ~/.kaggle/access_token
  (where the Kaggle SDK expects it):

    python -c "
    from pathlib import Path
    env = Path('.env').read_text()
    token = next(l.split('=',1)[1] for l in env.splitlines() if l.startswith('KAGGLE_API_TOKEN='))
    p = Path.home() / '.kaggle'
    p.mkdir(exist_ok=True)
    (p / 'access_token').write_text(token)
    print('Done:', p / 'access_token')
    "

STEP 3  Download the EyePACS zip from Kaggle
--------------------------------
  Option A — Kaggle CLI (downloads the full ~32 GB; you can interrupt it
              once you have enough data — see STEP 5):

    kaggle datasets download dreamer07/eyepacs \
        -p data/eyepacs_supplement/_download_tmp/

  Option B — Browser: go to https://www.kaggle.com/datasets/dreamer07/eyepacs,
             click Download, save the zip to:
               data/eyepacs_supplement/_download_tmp/eyepacs.zip
             You can interrupt the download after ~22 GB — the extractor
             handles partial zips.

  NOTE: If another teammate already ran this, the folder
        data/eyepacs_supplement/ may already be present in the repo.
        In that case skip directly to STEP 6.

STEP 4  Download the labels CSV
--------------------------------
  The labels file is NOT inside the zip — download it separately:
    https://www.kaggle.com/datasets/dreamer07/eyepacs  -> Files -> trainLabels.csv

  Save it to:
    data/eyepacs_supplement/_download_tmp/trainLabels.csv

  Columns expected: image (stem like "10003_left"), level (0-4).

STEP 5  Extract supplement images
--------------------------------
  Run the preparation script. It reads the (partial) zip + trainLabels.csv,
  extracts N images per class, preprocesses them with the same hybrid
  pipeline used in v6, and saves everything to data/eyepacs_supplement/.

    python prepare_eyepacs_supplement.py

  To change how many images per class to extract, edit the line:
    IMAGES_PER_CLASS = 250   (in prepare_eyepacs_supplement.py)
  Max safe values: Grade 4 has only 708 total in EyePACS.

  The original APTOS data is never modified.
  To fully undo: delete data/eyepacs_supplement/

STEP 6  Run training
--------------------------------
  python train_coral_focal_v8.py

  Results go to results/exp_v8/
================================================================================
"""
...
# -- CONFIG --------------------------------------------------------------------
CONFIG = {
    'model_name':           'densenet121_coral_v8',
    'num_classes':          5,
    'input_size':           224,
    'batch_size':           16,
    'num_epochs':           30,
    ...
    'data_dir':             './data/raw',
    'output_dir':           os.path.join('results', 'exp_v8'),
...
class PreprocessedDRDataset(Dataset):
    """
    Loads images from a DataFrame that carries an 'img_dir' column so that
...
def build_dataframes():
    """
    Returns (full_df, aptos_train_df, val_df, combined_train_df).
    Each dataframe carries an 'img_dir' column so the Dataset can load
    from the correct cache folder per row.
    """
    ...
    if not supp_path.exists():
        raise FileNotFoundError(
            ...
            "Follow STEPS 1-5 in the header of this file, then re-run.")
...
def _verify_supplement_cache():
    ...
    if n_cached < n_total:
        print(f"[WARN] Supplement cache incomplete: {n_cached}/{n_total}. Rebuilding ...")
        preprocess_and_cache(
...
def main():
    print("=" * 80)
    print("TRAINING v8: APTOS + EyePACS supplement, denseblock1-only freeze")
    print("  (v7 froze denseblock1+2; v8 restores denseblock1-only as in v5/v6)")
    print("=" * 80)
    ...
    print("\nValidation distribution (APTOS only -- directly comparable to v6/v7):")
    ...
    # KEY DIFFERENCE vs v7: freeze denseblock1 ONLY
    freeze_early_layers(model, blocks=('denseblock1',))

    # Alpha on original APTOS labels (not inflated by supplement)
    ...
    report_full = (
        "Classification Report -- exp_v8\n"
        "=" * 60 + "\n"
        "NOTE: validation is APTOS-only (20% stratified split, seed=42).\n"
        "Directly comparable to v6 and v7.\n"
        "EyePACS supplement added to training only.\n"
        "Freeze: denseblock1 ONLY (v7 froze denseblock1+2).\n"
        "=" * 60 + "\n\n"
        + report_str
        + "\nCombined training distribution after oversampling:\n"
    )
    for g, n in bal_dist.items():
    ...
    class_report_dict = classification_report(
        final_labels, final_preds, target_names=grade_names, output_dict=True)

    summary = {
        'model':          'DenseNet121 + independent CORAL projections (v8)',
        'experiment':     'exp_v8',
        'changes_vs_v7':  [
            'Freeze denseblock1 ONLY (v7 froze denseblock1+2)',
            'More trainable parameters -> better adaptation to expanded dataset',
        ],
